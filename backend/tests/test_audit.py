...
def test_infer_action_maps_known_routes():
    assert infer_action("POST", "/vms") == ("vm.create", "vm", None)
    assert infer_action("POST", "/vms/bulk") == ("vm.bulk_create", "vm", None)
    assert infer_action("PATCH", "/vms/42") == ("vm.update", "vm", "42")
    assert infer_action("DELETE", "/vms/7") == ("vm.delete", "vm", "7")
    assert infer_action("POST", "/vms/3/snapshots") == ("baseline.create", "baseline", "3")
    assert infer_action("POST", "/plans") == ("plan.create", "plan", None)
    assert infer_action("PUT", "/settings") == ("settings.update", "settings", None)
...
def test_audit_filters_by_action(client, mock_vm_payload):
    client.post("/vms", json=mock_vm_payload)
    client.put("/settings", json={"schedule_preset": "hourly"})

    r = client.get("/audit?action=vm.create")
    body = r.json()
    assert len(body) == 1
...
def test_audit_filters_by_resource_type(client, mock_vm_payload):
    ...
    r = client.get("/audit?resource_type=settings")
    body = r.json()
    assert len(body) == 1
...
def test_audit_endpoint_respects_limit(client):
    for i in range(5):
        client.post("/vms", json={"name": f"vm-{i}", "source_hostname": f"vm-{i}.local"})
...
def test_audit_orders_newest_first(client):
    client.post("/vms", json={"name": "vm-a", "source_hostname": "a.local"})
    client.post("/vms", json={"name": "vm-b", "source_hostname": "b.local"})
...
def test_audit_picks_up_x_actor_header(client, mock_vm_payload):
    client.post("/vms", json=mock_vm_payload, headers={"x-actor": "alice@corp"})
    audit = client.get("/audit").json()
...
def test_audit_logs_patch_with_resource_id(client, mock_vm_payload):
    ...
    client.patch(f"/vms/{created['id']}", json={"role": "updated"})

    r = client.get("/audit?action=vm.update").json()
    assert len(r) == 1
...
def test_audit_logs_bulk_create(client):
    ...
    audit = client.get("/audit?action=vm.bulk_create").json()
    assert len(audit) == 1
...
def test_audit_invalid_limit_rejected(client):
    ...
    assert r.status_code == 422
    r = client.get("/audit?limit=10000")
    assert r.status_code == 422
