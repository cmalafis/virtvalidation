...
// Storage-specific finding categories. Order intentionally puts the
// most-impactful (replication / access mode) above the diagnostic
// (snapshot capability / multipath) categories so the UI surfaces
// blocking issues first.
const CATEGORY_LABEL = {
  performance_tier_mismatch: "Performance Tier Mismatch",
  capacity_concern: "Capacity Concern",
  access_mode_mismatch: "Access Mode Mismatch",
  replication_loss: "Replication Loss",
  snapshot_capability: "Snapshot Capability",
  multipath_policy: "Multipath Policy",
...
export default function StorageReviewDetail() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data, status } = await fetchJSON(`/api/storage-reviews/${reviewId}`);
      if (status === 404) {
  ...
  const onAnalyze = async () => {
    try {
      await fetchJSON(`/api/storage-reviews/${reviewId}/analyze`, { method: "POST" });
  ...
  const onTriage = async (findingId, triage) => {
    try {
      await fetchJSON(`/api/storage-reviews/${reviewId}/findings/${findingId}`, {
  ...
  const onDelete = async () => {
    ...
    try {
      await fetchJSON(`/api/storage-reviews/${reviewId}`, { method: "DELETE" });
